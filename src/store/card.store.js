...
const { fetchCards, setLoading, setPage, setQuery } = actionsCreator.card
...
const actions = {
  ...
  nextCards: () => async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch(
        fetchCards({
          cards: [...new Set([...Object.values(currentCards), ...cards])],
        }),
      )
      dispatch(setPage({ page }))
      dispatch(setLoading({ loading: false }))
...
const cards = createSelector(
  ...
  cardsState =>
    orderBy(
      cardsState.ids.map(id => cardsState.cards[id]),
      ['name'],
    ),
)
